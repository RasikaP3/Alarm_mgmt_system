...
def update_alarm_lifecycle(alarm):
    ...
    if status == "OPEN":
        ...
        if not lifecycle.get("createdAt"):
            lifecycle["createdAt"] = current_time
        ...
        if not lifecycle.get("acknowledgedAt"):
            lifecycle["acknowledgedAt"] = current_time
        ...
        if not lifecycle.get("resolvedAt"):
            lifecycle["resolvedAt"] = current_time
...
def update_metrics(alarm):
    ...
    resolved = lifecycle.get("resolvedAt")
    ...
    if opened and resolved:
        try:
            ...
            mttr = (resolved_dt - opened_dt).total_seconds()
...
def orchestrator_agent(state: AlarmState):
    ...
    producer.send(
        SUPPRESSED_TOPIC,
        json.dumps(structured_alarm).encode("utf-8")
    )

    producer.flush()

    print(f"📤 Structured alarm sent → {alarm['alarmId']}")
...
def run_pipeline(classified_file, monitoring_file):
    ...
    for alarm in alarms:
        ...
        alarm = update_alarm_lifecycle(alarm)
